  ...
  useEffect(() => {
    Promise.all([
      ...
      if (tData.travel) {
        ...
        setSplitMemberIds(tData.travel.members?.map((m: any) => m.id) || [])
      }
      if (eData.expense) {
        ...
        if (parsed.length > 0) {
          setExtraPayers(parsed.map((p: any) => ({ memberId: p.memberId, amount: String(p.amount) })))
        }
      }
      setLoading(false)
  ...
  return (
    <Container maxWidth="md" sx={{ mt: 3, mb: 3 }}>
      <Box display="flex" alignItems="center" gap={1} mb={3}>
        ...
        <Typography variant="h5">{t('expense.editExpense')}</Typography>
      </Box>
        ...
        <CardContent>
            ...
            <Grid size={12}>
              ...
              <Box display="flex" alignItems="center" gap={2}>
                <Button variant="outlined" component="label">
                  {existingImage ? t('expense.changeFile') : t('expense.chooseFile')}
          ...
          <Box mt={3} display="flex" gap={1} justifyContent="flex-end">
            ...
            <Button variant="contained" onClick={handleSubmit} disabled={saving}>
              {saving ? <CircularProgress size={20} /> : t('expense.updateExpense')}
            </Button>
          </Box>
        </CardContent>
      </Card>
